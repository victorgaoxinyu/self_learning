    ...
    async def request(self, *args, **kwargs):
        if self.current_failures >= self.max_failures:
            if datetime.now() > self.last_request_time + timedelta(seconds=self.reset_interval):
                self._reset("Circult is going from open to closed, resetting!")
                return await self._do_request(*args, **kwargs)
            else:
                print("Circult is open, failing fast!")
                raise CircuitOpenException()
        else:
            if self.last_failure_time and datetime.now() > self.last_failure_time + timedelta(seconds=self.time_windows):
                self._reset('Interval since first failure elasped, resetting')
            print("Circuit is closed, reqeusting!")
            return await self._do_request(*args, **kwargs)
