...
FAVOURITE_BASE = 'http://127.0.0.1:8002'
...
async def all_products(request: Request) -> Response:
    async with aiohttp.ClientSession() as session:
        ...
        favourites = asyncio.create_task(session.get(f'{FAVOURITE_BASE}/users/3/favourites'))
        cart = asyncio.create_task(session.get(f'{CART_BASE}/users/3/cart'))

        requests = [products, favourites, cart]
        done, pending = await asyncio.wait(requests, timeout=1.0)

        if products in pending:
            # we cannot continue if products service timed out
            [request.cancel() for request in requests]
            return web.json_response({
                'error': 'Could not reach products service'
            }, status=504)
            ...
            return web.json_response({
                'error': 'Server error reaching products'
            }, status=500)
        else:
            # get data from product resp
            product_response = await products.result().json()
            product_results = await get_products_with_inventory(session, product_response)

            cart_item_count = await get_response_item_count(cart, done, pending, 'Error getting user cart.')
            favourites_item_count = await get_response_item_count(favourites, done, pending, 'Error getting user favourites.')
            return web.json_response({
                'cart_items': cart_item_count,
                'favourite_items': favourites_item_count,
